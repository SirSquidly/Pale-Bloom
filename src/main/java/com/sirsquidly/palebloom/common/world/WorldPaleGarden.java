package com.sirsquidly.palebloom.common.world;
...
public class WorldPaleGarden
{
    ...
    { alertReapingWillow(world, pos, target, distance, configPaleCloakEnabled && reapingWillowsIgnoreCrimes); }

    /** Call a nearby Reaping Willow to defend. */
    public static void alertReapingWillow(World world, BlockPos pos, EntityLivingBase target, int distance, boolean obeyCloakWearer)
    {
        if (!reapingWillowIsntApathetic || !target.isEntityAlive() || world.isRemote) return;
        boolean flag = true;
        ...
        for (EntityReapingWillow entityReapingWillow : world.getEntitiesWithinAABB(EntityReapingWillow.class, (new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1.0D, pos.getY() + 1.0D, pos.getZ() + 1.0D)).grow(distance, 10.0D, distance)))
        {
            ...
            {
                /* Ignore Pale Moss Cloak wearers.*/
                if (obeyCloakWearer && target.getItemStackFromSlot(EntityEquipmentSlot.CHEST).getItem() == JTPGItems.PALE_MOSS_CLOAK) return;
