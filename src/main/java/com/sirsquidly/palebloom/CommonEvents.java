...
import com.sirsquidly.palebloom.item.ItemPaleHoe;
...
public class CommonEvents
{
    ...
    private static final boolean configPaleCloakCreepersIgnore = Config.item.paleMossCloak.creepersIgnoreCloakWearers;
    private static final boolean configPaleCloakReapingDefends = Config.item.paleMossCloak.reapingWillowsDefendCloakWearers;

    /** Hitting the ground with a Pale Hoe throws Thorns. */
    @SubscribeEvent
    public static void onLeftClickWithPaleHoe(PlayerInteractEvent.LeftClickBlock event)
    {
        if (event.getWorld().isRemote || event.getFace() != EnumFacing.UP ) return;

        ItemStack stack = event.getItemStack();
        if (!stack.isEmpty() && stack.getItem() instanceof ItemPaleHoe)
        {
    ...
    {
        if (!configPaleCloakEnabled || !configPaleCloakCreepersIgnore) return;
